...
def test_asset_create(admin_client):
    ...
    assertFormError(response.context['form'], 'asset_id', ['This field is required.', 'An Asset ID can only consist of letters and numbers, with a final number'])
...
def test_cable_create(admin_client):
    ...
    assertFormError(response.context['form'], 'asset_id', 'An Asset ID can only consist of letters and numbers, with a final number')
    assertFormError(response.context['form'], 'cable_type', 'A cable must have a type')
    assertFormError(response.context['form'], 'length', 'The length of a cable must be more than 0')
    assertFormError(response.context['form'], 'csa', 'The CSA of a cable must be more than 0')
...
def assert_asset_form_errors(response):
    ...
    assertFormError(response.context['form'], 'status', 'This field is required.')
    assertFormError(response.context['form'], 'category', 'This field is required.')
    assertFormError(response.context['form'], 'date_sold', 'Cannot sell an item before it is acquired')
    assertFormError(response.context['form'], 'purchase_price', 'A price cannot be negative')
    assertFormError(response.context['form'], 'replacement_cost', 'A price cannot be negative')
